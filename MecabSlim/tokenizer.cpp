...
namespace MeCab {
    ...
    N* Allocator<N, P>::newNode() {
        N* node = node_freelist_->alloc();
        std::memset(node, 0, sizeof(N));
        node->id = Conversion::convertULongLongToUInt(id_++);
        return node;
    }
    namespace {
    ...
    bool Tokenizer<N, P>::open(const Param& param) {
        ...
        if (!userdic.empty()) {
            ...
            strncpy_s(buf.get(), buf.size(), userdic.c_str(), buf.size());
        ...
        for (int i = static_cast<int>(dic_.size() - 1); i >= 0; --i) {
            ...
            d->size = Conversion::convertULongLongToUInt(dic_[i]->size());
            d->lsize = Conversion::convertULongLongToUInt(dic_[i]->lsize());
            d->rsize = Conversion::convertULongLongToUInt(dic_[i]->rsize());
            d->type = dic_[i]->type();
    ...
    namespace {
        inline bool partial_match(const char* f1, const char* f2) {
            ...
            strncpy_s(buf1.get(), buf1.size(), f1, buf1.size());
            strncpy_s(buf2.get(), buf2.size(), f2, buf2.size());
...
//      result_node = new_node; } } while (0)
        ...
        ) const
    {
        do {
            ...
            for (size_t k = 0; k < size; ++k) {
                ...
                new_node->char_type = cinfo.default_type;
                new_node->surface = begin2;
                new_node->length = Conversion::convertLongLongToUShort(begin3 - begin2);
                new_node->rlength = Conversion::convertLongLongToUShort(begin3 - begin);
                new_node->stat = MECAB_UNK_NODE;
                new_node->bnext = *presult_node;
                if (unk_feature_.get()) new_node->feature = unk_feature_.get();
                if (isPartial && !is_valid_node(lattice, new_node)) { continue; }
                *presult_node = new_node;
            }
        } while (0);
    }
    ...
    N* Tokenizer<N, P>::lookup(const char* begin, const char* end,
        ...
        for (std::vector<Dictionary*>::const_iterator it = dic_.begin();
            ...
            for (size_t i = 0; i < n; ++i) {
                ...
                for (size_t j = 0; j < size; ++j) {
                    ...
                    new_node->length = Conversion::convertULongLongToUShort(daresults[i].length);
                    new_node->rlength = Conversion::convertULongLongToUShort(begin2 - begin + new_node->length);
                    new_node->surface = begin2;
        ...
        if (begin3 > end) {
            AddUnknown<isPartial>(cinfo, allocator, begin, begin2, begin3, &result_node, lattice);
            if (result_node) {
                return result_node;
            }
        }
