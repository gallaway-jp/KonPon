...
namespace MeCab {
    namespace {
        ...
        void TaggerImpl::set_theta(double theta) {
            theta_ = theta;
        }

        double TaggerImpl::theta() const {
            return theta_;
        }
        ...
        void LatticeImpl::set_result(const char* result) {
            ...
            for (size_t i = 0; i < surfaces.size(); ++i) {
                ...
                node->length = Conversion::convertULongLongToUShort(surfaces[i].size());
                node->rlength = Conversion::convertULongLongToUShort(surfaces[i].size());
                node->isbest = 1;
