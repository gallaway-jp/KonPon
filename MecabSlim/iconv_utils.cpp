...
namespace MeCab {
    ...
    bool Iconv::convert(std::string* str) {
        ...
        if (::MultiByteToWideChar(from_cp_, 0, str->c_str(), -1,
            wide_str.get(), Conversion::convertULongLongToInt(wide_len + 1)) == 0) {
            return false;
        }
        ...
        if (::WideCharToMultiByte(to_cp_, 0, wide_str.get(),
            Conversion::convertULongLongToInt(wide_len),
            encoded.get(), Conversion::convertULongLongToInt(output_len + 1),
            NULL, NULL) == 0) {
            return false;
        }
