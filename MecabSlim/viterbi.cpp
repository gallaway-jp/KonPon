...
namespace MeCab {
    ...
    bool Viterbi::initPartial(Lattice* lattice) {
        ...
        size_t newStrSize = lattice->size() + 1;
        char* str = allocator->partial_buffer(newStrSize);
        strncpy_s(str, newStrSize, lattice->sentence(), newStrSize);
    ...
    bool Viterbi::viterbi(Lattice* lattice) const {
        ...
        for (long pos = Conversion::convertULongLongToLong(len); pos >= 0; --pos) {
            if (end_node_list[pos]) {
